...
for my $file ( @sorted ) {
	my $hits = `ncbi-blast-2.2.29+/bin/blastn -task blastn-short -db ncbi-blast-2.2.29+/db/16SMicrobial -query $file -num_threads 9 -outfmt \"7 qacc sallseqid evalue bitscore pident qstart qend sstart send saccver\" -num_alignments $num_alignments | grep -v \"\#\" | perl ./blast_org_annotate2.pl`;
	#print "hits:\n$hits\n";
	my @besthits;
	my @maxhits;
	my @genuslist;
